...
const DuplicateUserProduct = (props) => {
    ...
    const transformArtworkData = (userProductArtworkFusionData, abstractProduct, variants, sideLayersData) => {
        return new Promise(async (resolve, reject) => {
            // console.log(userProductArtworkFusionData)
    ...
    const _fetchProductInfo = async () => {
        setLoading(true)
        setLoadingInfo(true)
        const productsRes = await retrieveDuplicateProductData(productId)
        ...
        const userProductData = await transformUserProduct(userProduct, abstractProduct, variants)

        const state = {
            product: {
                step: 0,
                defaultBackgroundColor: userProduct.background_color || DEFAULT_BACKGROUND_COLOR,
                originalUserProductId: productId,
                mockupVersion: mockupVersion,
                abstract_product_id: userProduct.abstract_product,
...
export default DuplicateUserProduct
