...
export const DestructiveConfirmModal = ({butttonType = POLARIS_TYPE, buttonText, buttonClass, title, content, primaryAction, secondaryActionText}) => {
    const [modalActive, setModalActive] = useState(false)
    const handleActiveModalChange = () => setModalActive(!modalActive)
    return (
        <div>
            {butttonType === POLARIS_TYPE &&
            <Button destructive onClick={handleActiveModalChange}>{buttonText}</Button>}
