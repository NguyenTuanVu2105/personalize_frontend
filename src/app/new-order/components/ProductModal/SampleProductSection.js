...
import {listSampleProduct, retrieveSampleProductVariants} from "../../../../services/api/newOrder"
...
import './SampleProductSection.scss'
...
const SampleProductSection = (props) => {
    ...
    const _fetchProducts = async (page, limit, query, since, until) => {
        _setLoading(true)
        const productsRes = await listSampleProduct(query, page || _page, limit || rowNumber, since, until)
    ...
    const fetchVariants = async (productId) => {
        ...
        const {success: variantSuccess, data: variantsData} = await retrieveSampleProductVariants(productId)
        if (!variantSuccess) {
            notification.error({
                message: "Error",
                description: "An error occurred, please try again or contact our support team"
        ...
        const variantData = variants.map(variant => {
            const {user_variant, user_product, title, preview, extra_cost, abstract_variant_data} = variant
            return {
        ...
        return (
            <div key={index} className={"user-variant-item flex-start"}>
                <Checkbox
                    ...
                    disabled={notChangeVariantIds && notChangeVariantIds.includes(variant.user_variant.toString())}
                    style={{
                        width: '100%',
                        display: "flex",
                        alignItems: "center",
                                ...
                                <div className="row mx-0">
                                    {
                                        tableProducts.length > 0 && tableProducts.map((product, index) => {
                                            const selectedVariantAmount = selectedVariants.filter(v => v.user_product === product.id || v.sample_id === product.id).length
...
export default SampleProductSection